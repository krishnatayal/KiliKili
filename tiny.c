...
int main(int argc, char **argv) {
  ...
  char wgiargs[BUFSIZE]; /* wgi argument list */
    ...
    if (!strstr(uri, "cgi-bin")) { /* static content */
      ...
      if (uri[strlen(uri)-1] == '/') 
        ...
        if (p) {
		 ...
		 int len = strlen(cgiargs); 
		 int temp1 = 0;
		 int temp2 = 0;
		 while(cgiargs[temp1] != '\0')
		 {
			if( (temp1 + 2 < len) && (cgiargs[temp1] == '%' && cgiargs[temp1+1] == '2' && cgiargs[temp1+2] == '0'))
		       	{	wgiargs[temp2++] = ' ';
				temp1 = temp1 + 3;		
 			}
			else
